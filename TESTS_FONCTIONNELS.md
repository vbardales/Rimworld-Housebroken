...
| TF-19, second game | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only. Repository `9b1e07d` | Two games in one process, reduced elephant then an animal given its ID | The animal of the second game had its own factor at once and 251 ticks later. **The transfer by caravan between two maps failed to start (no valid tile for a second settlement found); the step is being fixed.** Evidence: runs x-tf19-games and x-tf19-caravan | Second game Passed (Pickle); caravan Failed, in the test, not the mod | — |
| TF-16, TF-20, TF-21, TF-22, and the rest of TF-15, TF-18 and TF-19 | — | — | Scenarios written for Pickle, not yet run. See `TESTING.md`. | Not executed | — |
