...
Manual acceptance tests for RimWorld 1.6. Originally written on September 12, 2026 from the code and documentation; translated and extended on September 13. **TF-01 to TF-14 were played by Pickle in a headless game on September 25 (see the execution record). TF-15 to TF-22 followed on September 25 and 26, all passed under Pickle, with the limits listed in each row.** Existing scenario IDs and execution history are preserved.
...
| --- | --- | --- | --- | --- | --- |
| TF-01 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; no animal | Every default read back as documented; shortcut hidden and not greyed; no error logged. The colony is the fixture, not a newly started one. Evidence: `docs/runs/2026-09-25.md`, run `tf01-06` | Passed (Pickle) | — |
| TF-02 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; cow, rhinoceros, elephant | All seven rows of the table matched (1, 0.8, 0.6, 0.4, 0.3, 0.15, 0.15 times the base); the explanation names the training factor once and no location line; none for the cow. Evidence: run `tf01-06` | Passed (Pickle) | — |
| TF-03 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant | Rates 0.6, 0.6, 0.3, 0.15, 0.3, 0.6 in order, 251 game ticks after each change. Evidence: run `tf01-06` | Passed (Pickle) | — |
| TF-04 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant | 0.3 with the two sliders changed; base rate everywhere with every reduction at zero, no explanation, manure produced indoors and the animal still listed by the alert; zero with a 100% reduction. **The slider bounds in the interface (0 to 100%, 100 to 400%) were not played in game**: the offline normalization tests cover them. Evidence: run `tf01-06` | Passed (Pickle), one claim not executed | — |
| TF-05 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; colony, wild and other-faction elephants, colonist, mechanoid | Only the colony animal reduced; wild and other-faction animals reduced once colony-only is off; colonist and mechanoid always the base rate; joining and leaving the player faction followed at once. Evidence: run `tf01-06` | Passed (Pickle) | — |
| TF-06 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; cow, rhinoceros, elephant | Trainability moved one tier (none to intermediate, intermediate to advanced, advanced unchanged) and the factor followed: 0.8, 0.6, 0.6. No separate catalyst bonus. Evidence: run `tf01-06` | Passed (Pickle) | — |
| TF-07 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant, reduction 0.15 | Closed room 0; unroofed pen, roofed room outside the home area, open ground, doorway and a room touching the map edge all 0.3 times the base; holding and outside explanation lines correct; applied without waiting. Evidence: runs `fix-cache-tf07` and `tf07-12` | Passed (Pickle) | — |
| TF-08 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant | Whole-home mode: 0 in every home-area place, doorway and edge room included; 0.3 outside; removing and restoring the cell membership applied at once. Evidence: run `tf07-12` | Passed (Pickle) | — |
| TF-09 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant | 0.075 inside and 0.45 outside with 50% and 300%; 0.15 in both places with the rule off and no location line. Evidence: run `tf07-12` | Passed (Pickle) | — |
| TF-10 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant, cow control | Over 3000 steps of the game own entry point: no manure indoors by default, some outside, some again indoors with the rule off, and the control behaved normally. **The walking is a teleport from cell to cell**, not the pathfinder. Evidence: run `tf07-12` | Passed (Pickle), walking simulated | — |
| TF-11 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant, cow control; mud, then blood | Carried mud and carried blood, each on its own: held indoors, released outdoors, released indoors with clean feet off, released indoors by the control. Carried blood is a carried filth, not a bleeding wound. Evidence: run `tf07-12` | Passed (Pickle), walking simulated | — |
| TF-12 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant | The four combinations of manure outdoors and clean feet gave the four expected results; whole-home mode governed the feet with manure outdoors off, and in the unroofed pen. Evidence: run `tf07-12` | Passed (Pickle), walking simulated | — |
| TF-13 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; elephant, obedience only, rate 7.2 | Listed with the exemption off, gone with it on, back with it off again. Evidence: run `tf13-14` | Passed (Pickle) | — |
| TF-14 | WSL headless RimWorld 1.6 through Pickle; Biotech and Odyssey active (TF-05 and TF-06 ran); Harmony and Housebroken only. `Housebroken.dll` sha256 `8DF40BAE…`, steps `F8F65C68…`, repository `f441a99`. Game build not kept | Fixture `test-colony` plus the proving ground the suite builds; default settings unless the row says so; two reduced elephants, two cows | All four listed with the exemption off; only the cows with it on; every entry names its own target; a cow leaving the room left the list; none of the four listed at the end. **Clicking an entry was not simulated**: the names and targets are compared in code. Evidence: run `tf13-14` | Passed (Pickle), clicks not played | — |
| TF-15, in one game | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only. Repository `9b1e07d`; settings-pass tools (keyed click, screenshot mode) | Fixture 	est-colony, elephant; every setting changed | Every change applied on close (rate 0.76); the reset button opened the confirmation; going back changed nothing; confirming restored every default and closing applied them. **Persistence across a game restart is a separate pass, not run yet.** Evidence: run x-tf15-reset, capture eset-confirmation.jpg opened | Passed (Pickle), restart half not executed | — |
| TF-17 | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only. Repository `9b1e07d` | 	est-colony, reduced elephant carrying mud | After the game saved and reloaded: still carrying, rate 0 inside and 0.3 outside, no drop indoors, drop outdoors. The reload is the game own, in the same process. Evidence: run x-tf17-reload | Passed (Pickle), same-process reload | — |
| TF-18, first half | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only. Repository `9b1e07d` | 	est-colony, reduced elephant carrying mud | A game saved with the mod active holds no mention of Housebroken outside the mod list of its header. **Loading it without the mod is not run yet.** Evidence: run x-tf18-save | Passed (Pickle), second half not executed | — |
| TF-19, second game | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only. Repository `9b1e07d` | Two games in one process, reduced elephant then an animal given its ID | The animal of the second game had its own factor at once and 251 ticks later. The transfer by caravan between two maps passed on the replay of 2026-09-25 (repository `c75b890`, after two failures of the test, not of the mod): 0 in the closed room, readable on the caravan, 0.3 outdoors after arrival, 0 in the yard of the second map. The caravan is sent and received through the game's own caravan functions, not by dragging on the world map, and the second map is a settlement founded by the step. Evidence: runs x-tf19-games and y3-tf19-caravan | Second game and caravan Passed (Pickle) | — |
| TF-15, restart | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only | Fixture test-colony; settings written in one launch, read in the next | The written settings were read back after a full game restart. Evidence: run z-tf15-restart, docs/runs/2026-09-25.md | Passed (Pickle) | — |
| TF-16, page | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only; English and French | The settings page at 100 and 200 percent | Passed at 100 percent in both languages. **At 200 percent the fixed-size window is clipped (title and Close off-screen)**: a limit of the window, not compared with another mod. Evidence: z-tf16-page-en, z-tf16-page-fr | Passed (Pickle), 200 percent clipped | — |
| TF-16, tooltips | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only; English and French; developer mode off for the capture (screenshot mode hides tooltips) | Every tooltip hovered and captured, 10 per language | All 20 captures opened: each shows its own tooltip beside its row, text complete, French translated. The Intermediate slider has no tooltip (none defined). Some tooltips cover the row below, which is the game's own placement. Evidence: z2-tf16-tips-en, z2-tf16-tips-fr | Passed (Pickle) | — |
| TF-18, second half | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only | A game saved with the mod, loaded without Housebroken and its test mod | The game loaded and ran without the mod. Evidence: z-tf18-removal | Passed (Pickle) | — |
| TF-20 | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only; a companion mod on the same stat, loaded before and after Housebroken | Two launches | Both contributions to the filth rate stand in both load orders. Evidence: z-tf20-before, z2-tf20-after | Passed (Pickle) | — |
| TF-21 | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only; RIMMSQOL | Shortcut hidden by default, revealed, activated, then Mod options | Hidden and not greyed by default; revealed and drawn on the main bar (capture opened); activating it opens the settings and a value set there persists; the restart half passed (z-tf21-restart). **The main-bar tooltip is not observable**: the main bar records no tip regions. Repository ef79f60. Evidence: z2-tf21-shortcut | Passed (Pickle), tooltip not played | — |
| TF-22 | WSL headless RimWorld 1.6.4871 through Pickle; Biotech and Odyssey active; Harmony and Housebroken only | An older file with fields missing, and a file with out-of-range and non-finite numbers, each read after a restart | Missing fields take defaults; out-of-range values are clamped and non-finite ones take defaults. Evidence: z-tf22-old, z-tf22-range | Passed (Pickle) | — |
